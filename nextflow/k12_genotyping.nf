...
params.fasta_for_ref = "/nfs/leia/research/iqbal/rmcolq/data/references/Escherichia_coli/NC_000913.3.all.fa"
...
process pandora_get_vcf {
  ...
  file truth_assembly from reference_assembly
  file index from pandora_idx
  file kmer_prgs from pandora_kmer_prgs

  output:
  file("pandora/pandora_genotyped.vcf") into ref_vcf

  """
  pandora map -p ${prg} -r ${truth_assembly} --vcf_refs ${ref} --genotype
  """
}
...
process simulate_new_ref {
  ...
  output:
  file("pandora/pandora.consensus.vcf") into ref_vcf
  ...
  cat ${truth_assembly} | vcf-consensus simulated_vars.vcf.gz > corrected.fa
  """
} 
...
process pandora_map_nano {
  ...
  output:
  set file("pandora/pandora.consensus.fq.gz"), file("pandora/pandora_genotyped.vcf") into pandora_output_nano
