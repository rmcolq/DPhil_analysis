...
process pandora_get_ref_vcf {
    ...
    sed '/^@/!d;s//>/;N' pandora/pandora.consensus.fq > pandora/pandora_genotyped.ref.fa
    """
}
...
process simulate_new_ref {
    ...
    cat \$v | vcf-consensus simulated_vars.filtered.vcf.gz > simulated_ref.fa
    """
}
...
process pandora_genotype_nanopore {
    memory { 42.GB * task.attempt }
        ...
        """
        which pandora
        pandora map -p ${pangenome_prg} -r ${nanopore_reads} -w 14 -k 15 --genotype
        mv pandora/pandora_genotyped.vcf pandora_genotyped_full.vcf
        gunzip -c pandora/pandora.consensus.fq.gz | awk '{if(NR%4==1) {printf(">%s\n",substr($0,2));} else if(NR%4==2) print;}' > pandora_genotyped_full.ref.fa
        """
    }
    ...
    process pandora_genotype_30X_nanopore {
        ...
        """
        which pandora
        pandora map -p ${pangenome_prg} -r ${nanopore_reads} -w 14 -k 15 --genotype --max_covg 30
        mv pandora/pandora_genotyped.vcf pandora_genotyped_30X.vcf
...
if (params.mask) {
    process compare_vcfs {
        ...
        """
        python3 ${params.pipeline_root}/scripts/compare_genotypers_on_sample.py --truth_vcf ${truth_vcf} --truth_vcf_ref ${truth_vcf_ref} --directory ${final_outdir} --mask ${mask}        
        """
    }
}
else {
    process compare_vcfs {
        ...
        """
        python3 ${params.pipeline_root}/scripts/compare_genotypers_on_sample.py --truth_vcf ${truth_vcf} --truth_vcf_ref ${truth_vcf_ref} --directory ${final_outdir}
        """
    }   
}       
*/
